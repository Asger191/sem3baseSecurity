...
import dk.ek.persistence.daos.SecurityDAO;
...
import static dk.ek.utils.Utils.logger;

public class SecurityController implements dk.ek.security.ISecurityController {
    ...
    public Handler login() {
        return (Context ctx) -> {
            ...
            Set<String> roles = checkedUser.getRoles().stream()
                    .map(r -> r.getRolename())
    ...
    public Handler register() {
        return ctx -> {
            ...
            User foundUser = securityDAO.getVerifiedUser(createdUser.getUsername(), createdUser.getPassword());

            Set<String> roles = foundUser.getRoles().stream()
                    .map(r -> r.getRolename())
                    .collect(Collectors.toSet());

            UserDTO forToken = new UserDTO(foundUser.getUsername(), roles);

            String token = createToken(forToken);

            System.out.println("User created: " + foundUser.getUsername());
